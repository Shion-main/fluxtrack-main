...
For the current relational model, treat Django models and migrations as
authoritative. [`docs/db_schema.sql`](./db_schema.sql) is a 2026-07-07 snapshot and
does not contain the later operational-trust migrations.
...
| Reports / media | qrcode + Pillow, ReportLab, pypandoc; Django `FileSystemStorage` | private/generated bytes below `MEDIA_ROOT`; production uses backed-up EBS storage |
...
| **campus** | Physical spaces: `Building` → `Floor` → `Room`. Room carries case-sensitive resolver credentials and its in-service state. | `models.py`, `services.py` |
| **scheduling** | Academic terms/lifecycle, breaks, suspensions, recurring schedules, dated sessions, modality requests, import, materialization, reports, and jobs. | `resolver.py`, `services.py`, `jobs.py`, `schedule_ops.py`, `suspensions.py`, `term_lifecycle.py` |
| **verification** | Duty and verification: `Assignment`, authoritative `CheckerValidation`, and durable `CheckerReplayReceipt`. | `resolver.py`, `services.py` |
| **ops** | Bookings, notifications/push, audit, conflicts, job observability, shared database cache, policy, import staging, and weekly-report storage. | `availability.py`, `occupancy.py`, `notify.py`, `reports.py`, `import_staging.py` |
| **web** | The only HTTP surface. Role-gated views for all seven roles, HTML/htmx/limited JSON responses, and the PWA shell. | `scan.py`, `checker.py`, `faculty.py`, `dean.py`, `ifo.py`, `ifo_terms.py`, `guard.py`, `hr.py`, `sys.py` |
| **config** | Project settings, root URLconf, WSGI/ASGI. | `settings.py`, `urls.py` |

Current data-model detail lives in each app's `models.py` and migration history.
[`docs/db_schema.sql`](./db_schema.sql) remains useful only as an earlier DDL snapshot.
...
| **IFO Admin** | `/ifo/rooms`, `/ifo/buildings`, `/ifo/terms`, `/ifo/import`, `/ifo/suspensions`, `/ifo/breaks`, bookings, duties, reporting |
| **Dean** | `/dean/requests`, department dashboard, reports, and scorecards |
| **HR Admin** | `/hr/attendance` and CSV export |
| **Guard** | `/guard/monitor`, `/guard/rooms/<code>`, `/guard/locate` |
| **System Admin** | Django `/admin/` for SYS-01..03 and `/sys/jobs` for scheduler monitoring |
...
flowchart LR
    subgraph Proc["runscheduler process (APScheduler BlockingScheduler)"]
        ...
        J3["weekly_report<br/>cron Mon 06:00"]
        J4["push_outbox<br/>every N sec (policy)"]
        J5["maintenance<br/>daily 03:15"]
    end
    ...
    J4 --> Wrap
    J5 --> Wrap
    Wrap --> M1["materialize_sessions<br/>(extend horizon; skip breaks/suspensions)"]
    Wrap --> M2["sweep_no_shows + detect_room_conflicts"]
    Wrap --> M3["generate prior-week reports + startup backfill"]
    Wrap --> M4["send web-push outbox"]
    Wrap --> M5["prune operational rows + expired sessions"]
    Wrap --> DB[("SQL Server")]
```
...
- User-sensitive cadences are policy-driven (`sweep_interval_minutes` and
  `push_outbox_interval_seconds`), while materialization/report/maintenance use
  explicit operational schedules.
...
- **Server-rendered** Django templates styled with **Franken UI 2.1.2**, which
  delivers the shadcn design language and tokens to non-React HTML. Franken UI,
  htmx 2.0.6, and html5-qrcode 2.3.8 are vendored and license-pinned under
  `static/vendor/`; production has no frontend CDN dependency.
...
flowchart TB
    ...
    subgraph Instance["App instance (EC2 / dev machine)"]
        Web["Web path<br/>Nginx → Gunicorn → Django"]
    ...
    Media["EBS-backed MEDIA_ROOT<br/>(photos, private imports, reports)"]
...
- Static files are collected into manifest-hashed assets and served by Nginx in
  production (WhiteNoise remains the Django fallback). Public media exposure is
  limited to normalized profile photos; imports and reports use authorized views.
...
- Django's database-cache table is shared by every Gunicorn worker for rate limits
  and short-lived idempotency. Durable replay receipts prevent duplicate offline
  application even after cache loss.
...
*Reconciled with the current codebase in Phase 16. Django models and migrations are
the authoritative schema; [`docs/db_schema.sql`](./db_schema.sql) is an earlier snapshot.*
