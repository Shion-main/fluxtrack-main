...
- [x] **IFO-09**: IFO sees a dashboard of summary cards over a selectable range with faculty-scorecard drill-down (shared RPT aggregates). CLOSED 2026-07-19 by Phase 06.1: the SRS's Room Occupancy in session-hours card now renders alongside the other four (five-card row per D-07), backed by a real room-aware aggregate in scheduling/reporting.py.
